...
/*
* TASS-PVL WINDOWS REALSENSE CONSOLE APP
*
...
void processFrames()
{
	...
	float frameRate = 60;
	Mat imgIn = cv::Mat::zeros(frameSize, CV_8UC3);
	Mat imgGray;
	...
	for (;;)
	{
		...
		if (sample->color)
			imgIn = PXCImage2CVMat(sample->color, PXCImage::PIXEL_FORMAT_RGB24);

		cvtColor(imgIn, imgGray, COLOR_BGR2GRAY);
		if (imgGray.empty())
		{
			std::cerr << "Error: get gray image()" << std::endl;
			return;
		}
